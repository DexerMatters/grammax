...
pub struct GrammarStateAnalysis {
    ...
    pub left_rec_alias: Vec<Option<usize>>,
}
...
impl GrammarStateAnalysis {
    pub fn from_table(table: &RuleTable, start: usize) -> Self {
        ...
        let mut analysis = Self {
            ...
            is_trivial,
            left_rec_alias: compute_left_rec_aliases(table),
        };
...
fn compute_left_rec_aliases(table: &RuleTable) -> Vec<Option<usize>> {
    use NormalizedGrammarNode::*;

    fn is_direct_ref(node: &NormalizedGrammarNode, target: usize) -> bool {
        match node {
            Reference(ix) => *ix == target,
            Field(_, inner) => matches!(inner.as_ref(), Reference(ix) if *ix == target),
            Sequence(nodes) if nodes.len() == 1 => is_direct_ref(&nodes[0], target),
            _ => false,
        }
    }

    fn rule_has_direct_ref(rule: &NormalizedGrammarNode, target: usize) -> bool {
        match rule {
            Alternative(alts) => alts.iter().any(|n| is_direct_ref(n, target)),
            other => is_direct_ref(other, target),
        }
    }

    let mut aliases = vec![None; table.rules.len()];
    for (i, info) in table.left_rec.iter().enumerate() {
        if info.is_none() {
            continue;
        }

        let mut candidate: Option<usize> = None;
        for (j, rule) in table.rules.iter().enumerate() {
            if i == j {
                continue;
            }
            if rule_has_direct_ref(rule, i) {
                let is_named = !table.rule_names[j].starts_with('@');
                match candidate {
                    None => candidate = Some(j),
                    Some(prev) => {
                        if is_named && table.rule_names[prev].starts_with('@') {
                            candidate = Some(j);
                        }
                    }
                }
